...
## Current Plan & Steps
1.  **Add Inquiry Form**: Implement a contact form section using Formspree.
2.  **Implement Theme Switching**: Add a theme toggle button and CSS variables for light mode.
3.  **Define Structure (`index.html`)**: Update layout to include the theme toggle and inquiry form.
4.  **Apply Styles (`style.css`)**: Implement transitions, light mode theme overrides, and form styling.
5.  **Implement Logic (`main.js`)**:
...
6.  **Verification**: Ensure no runtime errors and the animations run smoothly.
